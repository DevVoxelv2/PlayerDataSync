...
public class ConfigManager {
    ...
    private void validateAndMigrateConfig() {
        ...
        if (config.getKeys(false).isEmpty()) {
            plugin.getLogger().warning("Configuration file is completely empty. Initializing with defaults...");
        ...
        if (ensureMissingConfigDefaults()) {
            changed = true;
        }
        ...
        if (changed) {
            plugin.getLogger().info("Configuration updated. Saving changes...");
            saveConfig();
        }
    }
     ...
     */
    private boolean ensureMissingConfigDefaults() {
        try (InputStream in = plugin.getResource("config.yml")) {
            ...
            return mergeMissingConfigKeys(config, defaults, "");
        } catch (Exception e) {
    ...
    private boolean mergeMissingConfigKeys(org.bukkit.configuration.ConfigurationSection target,
        ...
        for (String key : defaults.getKeys(false)) {
            ...
            if (defaults.isConfigurationSection(key)) {
                ...
                if (targetSection != null && defaultSection != null) {
                    if (mergeMissingConfigKeys(targetSection, defaultSection, fullPath)) {
                        changed = true;
                    }
                }
                continue;
